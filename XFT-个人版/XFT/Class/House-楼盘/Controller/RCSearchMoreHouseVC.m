...
@interface RCSearchMoreHouseVC ()<UITableViewDelegate,UITableViewDataSource>
...
@implementation RCSearchMoreHouseVC

- (void)viewDidLoad {
    [super viewDidLoad];
    [self.navigationItem setTitle:@"项目楼盘"];
    [self setUpTableView];
    [self setUpRefresh];
    [self startShimmer];
    [self getSearchRequest:YES];
}
...
-(void)setUpRefresh
{
    ...
    self.tableView.mj_header = [MJRefreshNormalHeader headerWithRefreshingBlock:^{
        ...
        [strongSelf getSearchRequest:YES];
    }];
    //追加尾部刷新
    self.tableView.mj_footer = [MJRefreshBackNormalFooter footerWithRefreshingBlock:^{
        hx_strongify(weakSelf);
        [strongSelf getSearchRequest:NO];
    }];
}
-(void)getSearchRequest:(BOOL)isRefresh
{
    ...
    [HXNetworkTool POST:HXRC_M_URL action:@"pro/pro/proBaseInfo/search" parameters:parameters success:^(id responseObject) {
        ...
        if ([responseObject[@"code"] integerValue] == 0) {
            if (isRefresh) {
                ...
                NSArray *arrt = [NSArray yy_modelArrayWithClass:[RCSearchHouse class] json:responseObject[@"data"]];
